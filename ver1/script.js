$(function () {
  ...
  $.fn.setFlexGrid = function (options) {
    var defaults = {
      ...
      cols: 4, // starting amount of columns across a grid
      rows: 7, // starting amount of rows in a grid
      fixedGrid: true, // determines whether the grid can be resized or not (adding more rows)
    ...
    $.fn.resetVars = function () {
      ...
      var rowHeight =
        options.rowHeight > 1 ? options.rowHeight * colWidth : colWidth; // height of each row
    ...
    $.fn.buildGrid = function () {
      ...
      for (var y = 0; y < rowAmount; y++) {
        for (var x = 0; x < colAmount; x++) {
          zoneInner.append(
            '<div class="fg-enabled-col fg-col ' +
              gridlines +
              '" data-fg-eq="' +
              x +
              '" data-fg-row="' +
          ...
          var appended = zoneInner.find(
            '.fg-col[data-fg-row="' + y + '"][data-fg-eq="' + x + '"]'
          );
      ...
      zoneInner.css({
        // reset the zone-inner width / height to perfect it.
    ...
    $.fn.setData = function () {
      ...
      var data_minWidth = parseFloat(
        Math.round(widget.width("minWidth") / re.cW)
      );
      var data_minHeight = parseFloat(
        Math.round(widget.height("minHeight") / re.rH)
      );
      var data_width =
        parseFloat(Math.round(wigW / re.cW)) < data_minWidth
          ? data_minWidth
          : parseFloat(Math.round(wigW / re.cW)); // number of columns a widget spans
      var data_height =
        parseFloat(Math.round(wigH / re.rH)) < data_minHeight
          ? data_minHeight
          : parseFloat(Math.round(wigH / re.rH)); // number of rows a widget spans
      var data_maxWidth = parseFloat(
        Math.round(widget.width("maxWidth") / re.cW)
      );
      var data_maxHeight = parseFloat(
        Math.round(widget.height("maxHeight") / re.rH)
      );
      var data_y = zoneCol.attr("data-fg-row"); // row the widget starts on ~ 0 indexed
      var data_x = zoneInner
        .find('.fg-col[data-fg-row="' + data_y + '"]')
        .index(widget.closest('.fg-col[data-fg-row="' + data_y + '"]')); // column # the widget starts on ~ 0 indexed
      widget.attr({
        "data-fg-width": data_width,
        "data-fg-height": data_height,
        "data-fg-x": data_x,
        "data-fg-y": data_y,
        "data-fg-minwidth": data_minWidth,
        "data-fg-minheight": data_minHeight,
        "data-fg-maxwidth": data_maxWidth,
        "data-fg-maxheight": data_maxHeight,
    ...
    $.fn.setOption = function (option, val) {
      ...
      switch (option) {
        ...
        case "x":
          ...
          var col = grid.find(
            '.fg-col[data-fg-eq="' + val + '"][data-fg-row="' + y + '"]'
          );
          widget.detach();
          col.append(widget);
          break;
        case "y":
          ...
          var col = grid.find(
            '.fg-col[data-fg-eq="' + x + '"][data-fg-row="' + val + '"]'
          );
          widget.detach();
          col.append(widget);
          break;
    ...
    $.fn.modCols = function (modifier) {
      ...
      for (var r = rowStart; r < rowEnd; r++) {
        for (var c = colStart; c < colEnd; c++) {
          var self = zoneInner.find(
            '.fg-col[data-fg-row="' + r + '"][data-fg-eq="' + c + '"]'
          );
          if (modifier == "enable") {
            self.addClass("fg-enabled-col").removeClass("fg-disabled-col");
          }
          if (modifier == "disable") {
            self.removeClass("fg-enabled-col").addClass("fg-disabled-col");
          }
        }
      }
    };
    ...
    $.fn.zoneOverflow = function () {
      ...
      var yCon =
        parseInt(col.attr("data-fg-row")) + data_height >
        parseInt(zoneInner.find(".fg-col").last().attr("data-fg-row")) + 1;
      var dif = (data_width + data_x - options.cols) * re.cW;

      if ($(ogParent).length > 0) {
        // if we dropped widget we should have an 'ogParent'
        if ((options.cols - data_x) * re.cW < widget.width("minWidth")) {
          // if overflowing zone on x-axis
          var de = col.find(".fg-widget").detach();
          $(ogParent).append(de);
          res = { obj: $(ogParent) };
        } else if (xCon) {
          // if widget is overflowing zone on x-axis but is not at min-width
          widget.css("width", widget.width() - dif);
        }
        res = { obj: col };
      } else if ($(ogParent).length <= 0) {
        // if we added new widget
        var goHere = col;
        while (xCon) {
          // detach the widget and append to the next open fg-enabled-col until it no longer overflows the zone width
          ...
          yCon = yCon =
            parseInt(goHere.attr("data-fg-row")) + data_height >
            parseInt(zoneInner.find(".fg-col").last().attr("data-fg-row")) + 1;
      ...
      if (yCon) {
        // if widget is overflowing zone height but is not at min-height
    ...
    $.fn.addWidget = function (params) {
      ...
      width = noParams
        ? options.defaultWidth * re.cW
        : params.width != undefined || params.width != null
        ? params.width * re.cW
        : options.defaultWidth * re.cW;
      height = noParams
        ? options.defaultHeight * re.rH
        : params.height != undefined || params.height != null
        ? params.height * re.rH
        : options.defaultHeight * re.rH;
      minWidth = noParams
        ? options.minWidth * re.cW
        : params.minWidth != undefined || params.minWidth != null
        ? params.minWidth * re.cW
        : options.minWidth * re.cW;
      minHeight = noParams
        ? options.minHeight * re.rH
        : params.minHeight != undefined || params.minHeight != null
        ? params.minHeight * re.rH
        : options.minHeight * re.rH;
      maxWidth = noParams
        ? options.maxWidth * re.cW
        : params.maxWidth != undefined || params.maxWidth != null
        ? params.maxWidth * re.cW
        : options.maxWidth * re.cW;
      maxWidth = maxWidth == 0 ? "unset" : maxWidth;
      maxHeight = noParams
        ? options.maxHeight * re.rH
        : params.maxHeight != undefined || params.maxHeight != null
        ? params.maxHeight * re.rH
        : options.maxHeight * re.rH;
      maxHeight = maxHeight == 0 ? "unset" : maxHeight;
      nextAxis = noParams
        ? options.nextAxis
        : params.nextAxis != undefined || params.nextAxis != null
        ? params.nextAxis
        : options.nextAxis;

      var amountNeeded =
        (options.rowHeight * re.rH * options.defaultHeight) / re.rH;
      amountNeeded =
        options.rowHeight > 1 ? amountNeeded : options.defaultHeight;
      ...
      var nested =
        options.nested == true
          ? $('<div class="fg-nested-container"></div>')
          : $("");
      var widget = noParams
        ? $(
            '<div class="fg-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget."></i><div class="fg-widget-inner fg-widget-handle"><div class="zone-txt text-center"></div></div></div>'
          )
        : params.widget === undefined || params.widget === null
        ? $(
            '<div class="fg-widget"><i class="fa fa-times remove-widget" title="remove this widget"></i><div class="fg-widget-inner fg-widget-handle"><div class="zone-txt text-center"></div></div></div>'
          )
        : params.widget;
      widget.find(".fg-widget-inner").append(nested);

      widget.css({
        // set widget style options
        width: width,
        "min-width": minWidth,
        "max-width": maxWidth,
        height: height,
      ...
      switch (axis) {
        case "x":
          zoneInner.find(".fg-col").each(function () {
            goHere = $(this);
            if (
              !goHere.hasClass("fg-disabled-col") &&
              goHere.find(".fg-widget").length == 0
            ) {
              goHere = $(this);
        ...
        case "y":
          if (x === undefined || x === null || x === "") {
            // if we still aren't passed an x
            findNextColumn("x", zoneInner);
          }
          goHere = zoneInner.find(
            '.fg-col[data-fg-row="' + y + '"][data-fg-eq="' + x + '"]'
          );
          ...
          if (goHere.length == 0) {
            ...
            goHere = goHere.hasClass("fg-disabled-col")
              ? goHere
                  .nextAll(
                    '.fg-enabled-col[data-fg-row="' +
                      y +
                      '"][data-fg-eq="' +
                      x +
                      '"]'
    ...
    $.fn.removeWidget = function (widget) {
      ...
      var originalContainer =
        widget.data("originalContainer") != undefined ||
        widget.data("originalContainer") != null
          ? widget.data("originalContainer")
          : zoneCol;
      if (!originalContainer.hasClass("fg-col")) {
        // if the widget came from a different container
    ...
    $.fn.createRow = function () {
      // add a row to the grid ~ used internally
      var zoneInner = $(this);
      var re = zoneInner.resetVars(); // reset zone variables
      ...
      for (var i = 0; i < colAmount; i++) {
        zoneInner.append(
          '<div class="fg-enabled-col fg-col ui-sortable ' +
            gridlines +
            '" data-fg-eq="' +
            i +
            '" data-fg-row="' +
        ...
        var blah = zoneInner.find(
          '.fg-col[data-fg-row="' + rowCount + '"][data-fg-eq="' + i + '"]'
        );
    ...
    $.fn.addRow = function (val) {
      // add specified number of rows outside of plugin...
      var zoneInner = $(this);
    ...
    $.fn.addRowHere = function (goHere, rowsNeeded) {
      // rows will be appended after specified columns row
      var zoneInner = $(this);
      var re = zoneInner.resetVars(); // reset zone variables
    ...
    $.fn.removeThisRow = function () {
      ...
      var rowCount =
        parseInt(zoneInner.find(".fg-col").last().attr("data-fg-row")) + 1; // number of rows found in the zone

      if (zoneCol.length > options.cols * options.rows) {
        zoneInner
          .find('.fg-col[data-fg-row="' + (rowCount - 1) + '"]')
          .remove();
        rowCount =
          parseInt(zoneInner.find(".fg-col").last().attr("data-fg-row")) + 1;
    ...
    $.fn.removeRow = function (val) {
      // remove specified number of rows outside of plugin...
      var zoneInner = $(this);
      val = val === undefined || val === null ? 1 : val;
      for (var i = 0; i < val; i++) {
      ...
      $(".fg-col").sortable({
        ...
        over: function (event, ui) {
          ...
          if (!widget.data("sortableItem").bindings.hasClass(".fg-col")) {
            ...
            $(this)
              ...
              .addClass("fg-widget-placeholder");
        ...
        receive: function (event, ui) {
          ...
          if (!ui.item.data("sortableItem").bindings.hasClass("fg-col")) {
            ...
            var ogHeight =
              ui.item.data("sortableItem").helperProportions.height;
            ui.item.data({
            ...
            ui.item.css({
              ...
              maxWidth:
                options.maxWidth != null && options.maxWidth != undefined
                  ? options.maxWidth * re.cW
                  : "unset",
              maxHeight:
                options.maxWidth != null && options.maxWidth != undefined
        ...
        stop: function (event, ui) {
          ...
          if (
            ui.item.closest(".fg-col").index(zoneCol) != ui.item.data("ogIndex")
          ) {
    ...
    function resize(widget) {
      widget.resizable({
        grid: [
          re.cW,
          options.rowHeight > 1 ? options.rowHeight * re.cW : re.cW,
        ],
        ...
        start: function (event, ui) {
          ...
          for (var i = 0; i < sibs.length; i++) {
            ...
            if (
              colSib.children(".fg-widget").length > 0 &&
              (colSib.offset().left ==
                zoneCol.offset().left + zoneCol.width() ||
                colSib.offset().top == zoneCol.offset().top + zoneCol.height())
            ) {
          ...
          ui.element.data({
            ogSize: {
              width: ui.element[0].offsetWidth,
              height: ui.element[0].offsetHeight,
        ...
        stop: function (event, ui) {
          ...
          if (
            widget[0].offsetWidth != widget.data("ogSize").width ||
            widget[0].offsetHeight != widget.data("ogSize").height
          ) {
    ...
    $.fn.checkCollision = function (type, colSib) {
      ...
      if (type == "resize") {
        ...
        for (var i = 0; i < colSib.length; i++) {
          ...
          if (
            item.hasClass("fg-disabled-col") &&
            item.attr("data-fg-eq") >= colStart &&
            item.attr("data-fg-eq") <= colEnd &&
            item.attr("data-fg-row") >= rowStart &&
            item.attr("data-fg-row") <= rowEnd
          ) {
            danger.push("");
          }
        }
    ...
    function dropCollision(colSib, col) {
      ...
      var con =
        (r1 > x2 && x1 < x2 && y1 < y2 && b1 > y2) ||
        (r1 > x2 && x1 < x2 && y1 == y2 && b1 == b2) ||
        (r1 > x2 && x1 < x2 && y1 <= y2 && b1 >= b2) ||
        (r1 > x2 && x1 < x2 && y1 >= y2 && b1 <= b2) ||
        (r1 > x2 && x1 < x2 && y1 < b2 && b1 > b2) ||
        (x1 < r2 && r1 > r2 && y1 < y2 && b1 > y2) ||
        (x1 >= x2 && r1 <= r2 && y1 < y2 && b1 > y2);
      // if the columns x-axis is colliding with the sibling columns x-axis
      var xCon =
        (y1 <= y2 && x1 < x2 && r1 > x2 && b1 > y2) ||
        (y1 < y2 && b1 > b2 && x1 < x2 && r1 > x2) ||
        (b1 >= b2 && y1 < b2 && x1 < x2 && r1 > x2) ||
        (y1 > y2 && b1 < b2 && x1 < x2 && r1 > x2) ||
        (y1 == y1 && b1 == b2 && x1 < x2 && r1 > x2);
      ...
      if ($(ogParent).length > 0) {
        // if we dragged from another column
        if (xCon && yCon) {
          // if both the x-axis and y-axis have collision
          var xConVal = x2 - x1;
          var yConVal = y2 - y1;
          if (xConVal > yConVal) {
            // find which axis has more collision and resize for that one
            xCollision();
          } else {
            yCollision();
          }
        } else if (xCon) {
          // if only x-axis collision, resize for x-axis
          xCollision();
        } else if (yCon) {
          // if only y-axis collision, resize for y-axis
          yCollision();
        }
        ...
        function xCollision() {
          ...
          widget.width(x2 - x1);
          if (x2 - x1 < widget.width("minWidth")) {
            // if new width is less than the minWidth ~
            var de = widget.detach();
        ...
        function yCollision() {
          ...
          widget.height(y2 - y1);
          if (y2 - y1 < widget.height("minHeight")) {
            // if new height is less than the minHeight ~
            var de = widget.detach();
        ...
        // if we added a new widget
        var goHere = col;
        while (con) {
          // detach the widget and append to the next open fg-enabled-col until there is no collision
          var de = goHere.find(".fg-widget").detach();
          goHere = goHere.nextAll(".fg-enabled-col").first();
          if (goHere.length == 0 || goHere == undefined || goHere == null) {
            zoneInner.createRow();
    ...
    function moreHeight(el) {
      // when a widget is appended but it overflows the zone height, append more rows until the height + the data-fg-row == the last rows 'data-fg-row'
      var zoneInner = el.closest(".flexgrid-grid");
      var widget = el.find(".fg-widget");
      var row_and_height =
        parseInt(el.attr("data-fg-row")) +
        parseInt(widget.attr("data-fg-height"));
      var last_row_num = parseInt(
        zoneInner.find(".fg-col").last().attr("data-fg-row")
      );
    ...
    $.fn.clearGrid = function () {
      ...
      for (var i = 0; i < widgets.length; i++) {
        ...
        if (!originalContainer.hasClass("fg-col")) {
          ...
          // if the widget was appended by `.addWidget()`
          widget.remove();
        }
      }
      zoneInner.html("");
    ...
    $.fn.getOption = function (option) {
      var res =
        option === undefined || option === null ? options : options[option];
      return res;
    };
  ...
  $(document).on("click", ".fg-add-widget", function () {
    ...
    var widget = $(
      '<div class="fg-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;"></div></div>'
    );
    zoneInner.addWidget({ widget: widget });
  });

  $(document).on("click", ".fg-add-widget-photo", function () {
    // widget.find('.fg-widget-inner').css('background', options.background != null ? options.background[Math.floor(Math.random() * options.background.length)] : '');
    var widget = $(
      '<div class="fg-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;"><img src="https://placekitten.com/g/200/300" alt="michi" style="width:100%;height:100%;"></div></div>'
    );
    zoneInner.addWidget({ widget: widget });
  });

  $(document).on("click", ".fg-add-widget-video", function () {
    // widget.find('.fg-widget-inner').css('background', options.background != null ? options.background[Math.floor(Math.random() * options.background.length)] : '');

    var content =
      '<iframe width="100%" height="100%" src="https://www.youtube-nocookie.com/embed/BRGwCgfS4Qk?autoplay=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
    var widget = $(
      '<div class="fg-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;">' +
        content +
        "</div></div>"
    );
    zoneInner.addWidget({ widget: widget });
  ...
  $(document).on("click", ".fg-add-widget-sticker", function () {
    // widget.find('.fg-widget-inner').css('background', options.background != null ? options.background[Math.floor(Math.random() * options.background.length)] : '');
    var content =
      '<img src="https://i.giphy.com/media/RtdRhc7TxBxB0YAsK6/giphy.gif" alt="michi" style="width:100%;height:100%;">';
    var widget = $(
      '<div class="fg-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;">' +
        content +
        "</div></div>"
  ...
  for (var i = 0; i < widgets.length; i++) {
    ...
    var widget = $(
      '<div class="fg-widget custom-blue-widget"><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;">' +
        inner +
        "</div></div>"
    );
    zoneInner.addWidget({
  ...
  $(".nested-holder").sortable({
    ...
    stop: function (event, ui) {
      ...
      if (
        ui.item.hasClass("cloner") &&
        !ui.item.parent(".nested-holder").length
      ) {
        $(this).append(clone);
      }
    },
